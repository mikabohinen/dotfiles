...
maps.n["<leader>q"] = {
	function()
		require("base.utils").confirm_quit()
	end,
	desc = "Quit",
}
maps.n["<Tab>"] = {
...
maps.n["n"] = {
	"nzzzv",
	desc = "Better next search result",
}
maps.n["N"] = {
	"Nzzzv",
	desc = "Better previous search result",
}
...
maps.n["<leader>c"] = { -- Close window and buffer at the same time.
	function()
		require("base.utils.buffer").wipe()
	end,
	desc = "Wipe buffer",
}
maps.n["<leader>C"] = { -- Close buffer keeping the window.
	function()
		require("base.utils.buffer").close()
	end,
	desc = "Close buffer",
}
...
maps.n["<leader>bc"] = {
	function()
		require("base.utils.buffer").close_all(true)
	end,
	desc = "Close all buffers except current",
}
maps.n["<leader>bC"] = {
	function()
		require("base.utils.buffer").close_all()
	end,
...
maps.n["<leader>bd"] = {
	function()
		require("base.utils.status").heirline.buffer_picker(function(bufnr)
			require("base.utils.buffer").close(bufnr)
		end)
	end,
...
maps.n["<leader>bl"] = {
	function()
		require("base.utils.buffer").close_left()
	end,
	desc = "Close all buffers to the left",
}
maps.n["<leader>br"] = {
	function()
		require("base.utils.buffer").close_right()
	end,
...
maps.n["<leader>bsr"] = {
	function()
		require("base.utils.buffer").sort("unique_path")
	end,
	desc = "Sort by relative path (buffers)",
}
maps.n["<leader>bsp"] = {
	function()
		require("base.utils.buffer").sort("full_path")
	end,
	desc = "Sort by full path (buffers)",
}
maps.n["<leader>bsi"] = {
	function()
		require("base.utils.buffer").sort("bufnr")
	end,
	desc = "Sort by buffer number (buffers)",
}
maps.n["<leader>bsm"] = {
	function()
		require("base.utils.buffer").sort("modified")
	end,
...
if is_available("nvim-coverage") then
	maps.n["<leader>Tc"] = {
		function()
			utils.notify("Attemping to find coverage/lcov.info in project root...", 3)
