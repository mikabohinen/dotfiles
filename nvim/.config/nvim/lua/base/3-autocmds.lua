...
--       -> 6. Java debugger.
--       -> 7. Spell checking for certain files
--       -> 8. Close some files with q
--
--       ## COOL HACKS
--       -> 9. Effect: URL underline.
--       -> 10. Customize right click contextual menu.
--       -> 11. Unlist quickfix buffers if the filetype changes.
--       -> 12. Close all notifications on BufWritePre.
--
--       ## COMMANDS
--       -> 13. Nvim updater commands.
--       -> 14. Neotest commands.
--       ->     Extra commands.
...
-- })
--
-- 8. Wrap and check for spell in text filetypes
autocmd("FileType", {
	group = augroup("wrap_spell"),
	pattern = { "gitcommit", "markdown", "latex", "tex", "txt", "text" },
	callback = function()
...
-- ## COMMANDS --------------------------------------------------------------
-- 13. Nvim updater commands
cmd("NvimChangelog", function()
	require("base.utils.updater").changelog()
end, { desc = "Check Nvim Changelog" })
cmd("NvimUpdatePlugins", function()
	require("base.utils.updater").update_packages()
end, { desc = "Update Plugins and Mason" })
cmd("NvimRollbackCreate", function()
	require("base.utils.updater").create_rollback(true)
end, { desc = "Create a rollback of '~/.config/nvim'." })
cmd("NvimRollbackRestore", function()
	require("base.utils.updater").rollback()
end, { desc = "Restores '~/.config/nvim' to the last rollbacked state." })
cmd("NvimFreezePluginVersions", function()
	require("base.utils.updater").generate_snapshot(true)
end, { desc = "Lock package versions (only lazy, not mason)." })
cmd("NvimUpdateConfig", function()
	require("base.utils.updater").update()
end, { desc = "Update Nvim distro" })
cmd("NvimVersion", function()
	require("base.utils.updater").version()
end, { desc = "Check Nvim distro Version" })
cmd("NvimReload", function()
	require("base.utils").reload()
end, { desc = "Reload Nvim without closing it (Experimental)" })
